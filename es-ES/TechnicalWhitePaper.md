...
El software EOS.IO no obliga a los productores de bloques a entregar cualquier mensaje a cualquier otra cuenta. Cada productor de bloque hace su propia medida subjetiva de la complejidad computacional y el tiempo requerido para procesar una transacción. Esto aplica si una transacción es generada por un usuario automáticamente o por un script.

En un blockchain puesto en marcha adoptando el software EOS.IO, a nivel de red todas las transacciones son facturadas a un costo de ancho de banda computaciones fijo independientemente de si tomo .01ms o 10ms para ejecutarlo. Sin embargo, cada productor de bloques usando el software puede calcular el uso de los recursos utilizando su propio algoritmo y mediciones. Cuando un productor de bloques concluye que una transacción o cuenta ha consumido un monto desproporcionado de capacidad computacional ellos simplemente rechazan la transacción al momento de producir su propio bloque; sin embargo, todavía se procesará la transacción si otros productores de bloques la consideran válidas.
