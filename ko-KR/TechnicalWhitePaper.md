...
EOS.IO 소프트웨어의 큰 장점 중 하나는 애플리케이션에서 사용할 수 있는 대역폭의 양이 토큰 가격에 완전히 독립적이라는 것입니다. 만약에 애플리케이션 소유자가 충분한 양의 토큰을 가지고 있다면, 애플리케이션은 할당된(fixed) 상태와 대역폭 사용량의 범위 안에서 제한 없이 실행됩니다. 이 경우, 개발자와 사용자는 토큰 시장의 가격 변동에 영향을 받지 않으므로 가격 추이에 의존하지 않습니다. 다른 말로, EOS.IO 소프트웨어에서 블록 생산자는 토큰의 가치와 무관하게 자연스럽게 대역폭, 연산 능력, 저장 능력을 향상할 수 있습니다.
