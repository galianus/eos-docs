...
시간은 보안의 핵심 요소입니다. 대부분의 경우, 개인 키(private key)가 도난당한 경우 사용되기 전에는 이를 알기가 어렵습니다. 인터넷에 연결된 컴퓨터에 보관되는 키를 사용하는 애플리케이션을 일상적으로 사용하는 사람들에게 시간 기반의 보안(time based security)은 더욱 중요합니다. EOS.IO 소프트웨어는 애플리케이션 개발자가 특정 메시지가 블록에 포함되었지만 적용되기 이전에 지정한 시간 만큼 기다리도록 할 수 있습니다. 지정한 시간 내에서 메시지는 취소 가능합니다.
